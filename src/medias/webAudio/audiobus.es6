...
export default (nodefony) => {
  ...
  class AudioBus extends nodefony.Service {
    ...
    constructor(name, mixer, settings) {
      ...
      this.audioContext = new nodefony.webAudio.audioContext();
      this.tracks = [];
      this.nbTracks = 0;
      this.audioNodes = {};
      this.in = null;
    ...
    createTrack(media, settings) {
      var track = new nodefony.webAudio.Track(media, this, settings);
    ...
    removeTrack(track) {
      ...
      let name = null;
      switch (true) {
        case track instanceof nodefony.webAudio.Track:

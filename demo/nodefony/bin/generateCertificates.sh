...
cp -r $ROOT_CA/certs/ca.cert.pem $ROOT_DIR/ca/demo-root-ca.crt.pem
cp -r $ROOT_CA/private/ca.key.pem $ROOT_DIR/ca/demo-root-ca.key.pem
...
cp -r $ROOT_CA_INTERMEDIATE/private/demo.key.pem $ROOT_DIR/server/privkey.pem
cp -r $ROOT_CA_INTERMEDIATE/certs/server.demo.cert.pem $ROOT_DIR/server/cert.pem
